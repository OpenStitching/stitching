...
from .context import FeatureDetector, FeatureMatcher, Subsetter
...
class TestImageRegistration(unittest.TestCase):
    ...
    def test_feature_matcher(self):
        ...
        detector = FeatureDetector("orb")
        features = [detector.detect_features(img1), detector.detect_features(img2)]
        ...
        self.assertTrue(
            np.array_equal(conf_matrix > 2, np.array([[False, True], [True, False]]))
        )
    ...
    def test_subsetting(self):
        ...
        self.assertEqual(pairwise_matches[13].confidence, matches_subset[1].confidence)
