...
from context import test_input, tutorial_output, write_tutorial_result, Images, FeatureDetector, FeatureMatcher, Subsetter, CameraEstimator, CameraAdjuster, WaveCorrector, Warper, Timelapser, Cropper, SeamFinder, ExposureErrorCompensator, Blender 
...
class TestStitcherTutorial(unittest.TestCase):
    def test_tutorial(self):
        ...
        for idx, img_features in enumerate(features):
            ...
            write_tutorial_result(f"tutorial_01_features_img{idx+1}.jpg", img_with_features)
        ...
        all_relevant_matches = list(matcher.draw_matches_matrix(medium_imgs, features, matches, conf_thresh=1, 
                                                           inliers=True, matchColor=(0, 255, 0)))
        for idx1, idx2, img in all_relevant_matches:
            write_tutorial_result(f"tutorial_02_matches_img{idx1+1}_to_img{idx2+1}.jpg", img)
        ...
        for idx, (img, corner) in enumerate(zip(cropped_final_imgs, final_corners)):
            timelapser.process_frame(img, corner)
            frame = timelapser.get_frame()
            write_tutorial_result(f"tutorial_07_timelapse_cropped_img{idx+1}.jpg", frame)
        ...
        for idx, compensated_img in enumerate(compensated_imgs):
            write_tutorial_result(f"tutorial_08_compensated{idx+1}.jpg", compensated_img)
        ...
        with_seam_lines = seam_finder.draw_seam_lines(panorama, blended_seam_masks, linesize=3)
        with_seam_polygons = seam_finder.draw_seam_polygons(panorama, blended_seam_masks)
        
        write_tutorial_result(f"tutorial_09_result_with_seam_lines.jpg", with_seam_lines)
        write_tutorial_result(f"tutorial_09_result_with_seam_polygons.jpg", with_seam_polygons)
