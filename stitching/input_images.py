...
class InputImages:
    ...
    def set_resolutions(
        ...
        self.resolutions = enum.Enum('Resolutions', {'MEDIUM':medium_megapix, 'LOW':low_megapix, 'FINAL':final_megapix})
    ...
    def read_and_resize(self, resolution):
        ...
        for idx, name in enumerate(self.img_names):
            ...
            if not self.scales_set:
                self.scalers["MEDIUM"] = MegapixDownscaler(self.resolutions.MEDIUM.value)
            ...
            yield InputImages.resize_img_by_scaler(self.get_scaler(resolution), size, img)
    ...
    def get_ratio(self, from_resolution, to_resolution):
        ...
        return self.get_scaler(to_resolution).scale / self.get_scaler(from_resolution).scale
    
    def get_scaled_img_sizes(self, resolution):
        assert isinstance(resolution, enum.Enum)
        return [self.get_scaler(resolution).get_scaled_img_size(sz) for sz in self.img_sizes]
